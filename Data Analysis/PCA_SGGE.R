...
SG_CRC_GE <- read.csv("SingaporeCRC_data/MSIde_exp.csv")
SG_CRC_GE_t <- t(SG_CRC_GE)
# Assign the first row as column names
colnames(SG_CRC_GE_t) <- SG_CRC_GE[, 1]
# Remove the first row since it is now the column names
SG_CRC_GE_t <- SG_CRC_GE_t[-1, ]
...
X <- SG_CRC_GE[, !names(SG_CRC_GE) %in% c('patient_id', 'TMB', 'KRAS', 'BRAF', 'NRAS', 'TP53', 'APC', 'PIK3CA',
...
# Visualize PCA for MSI Status
fviz_pca_ind(pca_result,
             geom.ind = "point",  # Show points only (no text)
             col.ind = SG_CRC_GE$MSI.Status,  # Color by groups (replace "Group" with your column name)
...
tsne_data$Group <- SG_CRC_GE$MSI.Status  # Assuming 'MSI.Status' is your grouping variable
...
# Visualize PCA for iCMS
fviz_pca_ind(pca_result,
             geom.ind = "point",  # Show points only (no text)
             col.ind = SG_CRC_GE$iCMS,  # Color by groups (replace "Group" with your column name)
             palette = c("#00AFBB", "#FC4E07"),
             addEllipses = TRUE,  # Add concentration ellipses
             legend.title = "Group"  # Adjust the legend title
) +
...
fviz_pca_ind(pca_result,
             geom.ind = "point",  # Show points only (no text)
             col.ind = SG_CRC_GE$Site.of.Primary.Colorectal.tumour,  # Color by groups (replace "Group" with your column name)
...
# Prepare t-SNE result for plotting
tsne_data <- data.frame(tsne_result$Y)
tsne_data$Group <- SG_CRC_GE$Site.of.Primary.Colorectal.tumour  # Assuming 'MSI.Status' is your grouping variable
colnames(tsne_data) <- c("TSNE1", "TSNE2", "Group")
...
fviz_pca_ind(pca_result,
             geom.ind = "point",  # Show points only (no text)
             col.ind = SG_CRC_GE$Vital.status,  # Color by groups (replace "Group" with your column name)
             palette = c("#00AFBB", "#FC4E07"),
             addEllipses = TRUE,  # Add concentration ellipses
             legend.title = "Group"  # Adjust the legend title
) +
...
# Prepare t-SNE result for plotting
tsne_data <- data.frame(tsne_result$Y)
tsne_data$Group <- SG_CRC_GE$Vital.status  # Assuming 'MSI.Status' is your grouping variable
colnames(tsne_data) <- c("TSNE1", "TSNE2", "Group")
