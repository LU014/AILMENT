...
# Visualize PCA for BLCA-gender
fviz_pca_ind(pca_BLCA,
             geom.ind = "point",  # Show points only (no text)
             col.ind = BLCA$gender,  # Color by groups (replace "Group" with your column name)
...
fviz_pca_ind(pca_BRCA,
             geom.ind = "point",  # Show points only (no text)
             col.ind = BRCA$gender,  # Color by groups (replace "Group" with your column name)
             palette = c("#00AFBB", "#FC4E07"),
             addEllipses = TRUE,  # Add concentration ellipses
             legend.title = "Group"  # Adjust the legend title
) +
...
fviz_pca_ind(pca_COAD,
             geom.ind = "point",  # Show points only (no text)
             col.ind = COAD$gender,  # Color by groups (replace "Group" with your column name)
             palette = c("#00AFBB", "#FC4E07"),
             addEllipses = TRUE,  # Add concentration ellipses
             legend.title = "Group"  # Adjust the legend title
) +
...
fviz_pca_ind(pca_GBM,
             geom.ind = "point",  # Show points only (no text)
             col.ind = GBM$gender,  # Color by groups (replace "Group" with your column name)
             palette = c("#00AFBB", "#FC4E07"),
             addEllipses = TRUE,  # Add concentration ellipses
             legend.title = "Group"  # Adjust the legend title
) +
...
tsne_data_GBM$Group <- GBM$gender
colnames(tsne_data_GBM) <- c("TSNE1", "TSNE2", "Group")

color_palette_GBM <- c("#00AFBB", "#FC4E07")
