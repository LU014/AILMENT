...
SG_FS_CRC_BA <- read.csv("SingaporeCRC_data/SG_FS_CRC_BA")
stage_mapping <- c(
  'I' = 'stage I',
  'II' = 'stage II',
  'III' = 'stage III',
...
SG_FS_CRC_BA <- SG_FS_CRC_BA %>%
  mutate(Stage = recode(Stage, !!!stage_mapping))

X <- SG_FS_CRC_BA[, !names(SG_FS_CRC_BA) %in% c('patient_id','TMB', 'KRAS', 'BRAF', 'NRAS', 'TP53', 'APC', 'PIK3CA', 'PIK3R1',
...
# Visualize PCA for Stage
fviz_pca_ind(pca_result,
             geom.ind = "point",  # Show points only (no text)
             col.ind = SG_FS_CRC_BA$Stage,  # Color by groups (replace "Group" with your column name)
...
fviz_pca_ind(pca_result,
             geom.ind = "point",  # Show points only (no text)
             col.ind = SG_FS_CRC_BA$MSI.Status,  # Color by groups (replace "Group" with your column name)
...
fviz_pca_ind(pca_result,
             geom.ind = "point",  # Show points only (no text)
             col.ind = SG_FS_CRC_BA$Site.of.Primary.Colorectal.tumour,  # Color by groups (replace "Group" with your column name)
