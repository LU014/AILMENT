...
SG_CRC_BA <- read.csv("SingaporeCRC_data/SG_CRC_BA")
X <- SG_CRC_BA[, !names(SG_CRC_BA) %in% c('TMB', 'KRAS', 'BRAF', 'NRAS', 'TP53', 'APC', 'PIK3CA', 'PIK3R1',
...
fviz_pca_ind(pca_result,
             ...
             col.ind = SG_CRC_BA$MSI.Status,  # Color by groups (replace "Group" with your column name)
...
tsne_data$Group <- SG_CRC_BA$MSI.Status  # Assuming 'MSI.Status' is your grouping variable
colnames(tsne_data) <- c("TSNE1", "TSNE2", "Group")

# Visualize PCA with iCMS
fviz_pca_ind(pca_result,
             geom.ind = "point",  # Show points only (no text)
             col.ind = SG_CRC_BA$iCMS,  # Color by groups (replace "Group" with your column name)
             palette = c("#00AFBB", "#FC4E07"),
             addEllipses = TRUE,  # Add concentration ellipses
             legend.title = "Group"  # Adjust the legend title
) +
...
fviz_pca_ind(pca_result,
             geom.ind = "point",  # Show points only (no text)
             col.ind = SG_CRC_BA$Site.of.Primary.Colorectal.tumour,  # Color by groups (replace "Group" with your column name)
...
# Prepare t-SNE result for plotting
tsne_data <- data.frame(tsne_result$Y)
tsne_data$Group <- SG_CRC_BA$Site.of.Primary.Colorectal.tumour  # Assuming 'MSI.Status' is your grouping variable
colnames(tsne_data) <- c("TSNE1", "TSNE2", "Group")
...
fviz_pca_ind(pca_result,
             geom.ind = "point",  # Show points only (no text)
             col.ind = SG_CRC_BA$Vital.status,  # Color by groups (replace "Group" with your column name)
             palette = c("#00AFBB", "#FC4E07"),
             addEllipses = TRUE,  # Add concentration ellipses
             legend.title = "Group"  # Adjust the legend title
) +
...
# Prepare t-SNE result for plotting
tsne_data <- data.frame(tsne_result$Y)
tsne_data$Group <- SG_CRC_BA$Vital.status  # Assuming 'MSI.Status' is your grouping variable
colnames(tsne_data) <- c("TSNE1", "TSNE2", "Group")
