...
SG_Gold_CRC_BA <- read.csv("SingaporeCRC_data/SG_Gold_CRC_BA")
X <- SG_Gold_CRC_BA[, !names(SG_Gold_CRC_BA) %in% c('patient_id', 'TMB', 'KRAS', 'BRAF', 'NRAS', 'TP53', 'APC', 'PIK3CA',
...
fviz_pca_ind(pca_result,
             ...
             col.ind = SG_Gold_CRC_BA$iCMS,  # Color by groups (replace "Group" with your column name)
...
tsne_data$Group <- SG_Gold_CRC_BA$iCMS  # Assuming 'MSI.Status' is your grouping variable
...
fviz_pca_ind(pca_result,
             geom.ind = "point",  # Show points only (no text)
             col.ind = SG_Gold_CRC_BA$MSI.Status,  # Color by groups (replace "Group" with your column name)
...
# Prepare t-SNE result for plotting
tsne_data <- data.frame(tsne_result$Y)
tsne_data$Group <- SG_Gold_CRC_BA$MSI.Status  # Assuming 'MSI.Status' is your grouping variable
colnames(tsne_data) <- c("TSNE1", "TSNE2", "Group")
...
fviz_pca_ind(pca_result,
             geom.ind = "point",  # Show points only (no text)
             col.ind = SG_Gold_CRC_BA$Site.of.Primary.Colorectal.tumour,  # Color by groups (replace "Group" with your column name)
...
# Prepare t-SNE result for plotting
tsne_data <- data.frame(tsne_result$Y)
tsne_data$Group <- SG_Gold_CRC_BA$Site.of.Primary.Colorectal.tumour  # Assuming 'MSI.Status' is your grouping variable
colnames(tsne_data) <- c("TSNE1", "TSNE2", "Group")
...
fviz_pca_ind(pca_result,
             geom.ind = "point",  # Show points only (no text)
             col.ind = SG_Gold_CRC_BA$Vital.status,  # Color by groups (replace "Group" with your column name)
             palette = c("#00AFBB", "#FC4E07"),
             addEllipses = TRUE,  # Add concentration ellipses
             legend.title = "Group"  # Adjust the legend title
) +
...
# Prepare t-SNE result for plotting
tsne_data <- data.frame(tsne_result$Y)
tsne_data$Group <- SG_Gold_CRC_BA$Vital.status  # Assuming 'MSI.Status' is your grouping variable
colnames(tsne_data) <- c("TSNE1", "TSNE2", "Group")
